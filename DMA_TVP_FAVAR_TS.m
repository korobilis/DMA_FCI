% DMA_TVP_FAVAR_TS - Time-varying parameters factor-augmented VAR using an adaptive Kalman filter
% with EWMA filter covariance estimation.
% MULTIPLE MODEL CASE (DYNAMIC MODEL AVERAGING - DMA): TRAINING SAMPLE (TS) INFORMATIVE PRIOR.
%-----------------------------------------------------------------------------------------
% The model is:
%     _    _     _              _     _    _     _    _
%    | y[t] |   |   I        0   |   | y[t] |   |   0  |
...
%  - This code is set to use a TRAINING SAMPLE PRIOR. You can still obtain the noninformative
%    prior case with a few amendments.
...
xdata = standardize_miss(xdata) + 1e-50 ;
xdata(isnan(xdata)) = 0;
ydata = standardize(ydata);

% Number of observations and dimension of X and Y
t = size(ydata,1); % t time series observations
n = size(xdata,2); % n series from which we extract factors
p = size(ydata,2); % and p macro series
...
% =========================| PRIORS |================================
% Obtain training sample, in order to from priors
X_tr = standardize_miss(xdata(1:tau,:)) + 1e-40; X_tr(isnan(X_tr)) = 0; Y_tr = ydata(1:tau,:);  % Training data
[F_tr,L_tr] = extract(X_tr,1);            % Principal component
YX_tr = [Y_tr,X_tr];                      % Y,X
YF_tr = [Y_tr,F_tr];                      % Y,F
[L,bb,beta_OLS,sigma2,sigmaf] = ols_pc_dfm(YX_tr,YF_tr,L_tr,y_true,n,p,r,nfac,nlag);  % Obtain OLS estimates

% Correct sizes after obtaining training sample
X_sam = xdata(tau+1:end,:); Y_sam = ydata(tau+1:end,:);
t = size(Y_sam,1);                % new sample size (t = t_full - tau)
yearlab = yearlab(tau+1:end);
t0 = find(strcmp(yearlab,t0)==1); % Conver t0 to numeric value
...
factor_0.mean = zeros(k,1);   factor_0.mean(1:r) = YF_tr(end,:);
factor_0.var = 4*eye(k);      factor_0.var(1:r,1:r) = sigmaf;
...
beta_0.mean = 0.9*bb;%b_prior;
beta_0.var = Vb_prior;
% Initial condition on the covariance matrices
V_0 = 0.1*eye(q) + sigma2; V_0(1:p,1:p) = 0;
Q_0 = sigmaf;
...
Yraw_f = [ydata(tau+1:end,:) ; NaN(nfore,p)];
PL = zeros(p,KK);
...
for irep = t0+1:t-1
    ...
    X_st = standardize_miss(X_sam(1:irep,:));% + 1e-20;
    X_st(isnan(X_st)) = 0;
